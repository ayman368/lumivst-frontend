...
async function getAllStocks() {
  ...
  try {
    while (true) {
      ...
      const response = await fetch(`lumivst-frontend.vercel.app/stocks?page=${currentPage}&limit=100&remove_duplicates=true`, {
        cache: 'no-store'
      })
...
async function getStockData(symbol: string, period: string = "annual") {
  ...
  try {
    ...
    try {
      const directRes = await fetch(`lumivst-frontend.vercel.app/stocks/${cleanSym}`, {
        cache: 'no-store'
      })
