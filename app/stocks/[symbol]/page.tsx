...
async function getAllStocks() {
  ...
  try {
    while (true) {
      ...
      const response = await fetch(`https://lumivstbackendv2-production.up.railway.app/stocks?page=${currentPage}&limit=100&remove_duplicates=true`, {
        cache: 'no-store'
      })
...
async function getStockData(symbol: string, period: string = "annual", country: string = "Saudi Arabia") {
  ...
  try {
    ...
    try {
      const directRes = await fetch(`https://lumivstbackendv2-production.up.railway.app/stocks/${cleanSym}?country=${encodeURIComponent(country)}`, {
        cache: 'no-store'
      })
    ...
    const [incomeRes, balanceRes, cashflowRes] = await Promise.all([
      fetch(`https://lumivstbackendv2-production.up.railway.app/financials/income_statement/${cleanSym}?country=${encodeURIComponent(country)}&period=${period}&limit=6`),
      fetch(`https://lumivstbackendv2-production.up.railway.app/financials/balance_sheet/${cleanSym}?country=${encodeURIComponent(country)}&period=${period}&limit=6`),
      fetch(`https://lumivstbackendv2-production.up.railway.app/financials/cash_flow/${cleanSym}?country=${encodeURIComponent(country)}&period=${period}&limit=6`)
    ])
