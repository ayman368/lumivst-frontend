...
function RatingBadge({ value }: { value?: string }) {
    ...
    if (val.startsWith('A')) {
        bg = '#86efac'; // Green-300 (Visible Green)
        border = '#4ade80';
...
export default function StocksPage() {
    ...
    const exportToCSV = () => {
        ...
        const allColumns = [
            ...
            { key: 'rs_rating', label: 'RS Rating', getValue: (s: Stock) => s.rs_rating || '' },
            { key: 'acc_dis_rating', label: 'A/D Rating', getValue: (s: Stock) => s.acc_dis_rating || '' },
            { key: 'industry_group_rs', label: 'Grp RS', getValue: (s: Stock) => s.industry_group_rs || '' },
            { key: 'sector_rs', label: 'Sec RS', getValue: (s: Stock) => s.sector_rs || '' },
            { key: 'industry_rs', label: 'Ind RS', getValue: (s: Stock) => s.industry_rs || '' },
            { key: 'sub_industry_rs', label: 'Sub-Ind RS', getValue: (s: Stock) => s.sub_industry_rs || '' },
                ...
                <table className={styles.table}>
                    <thead>
                        <tr>
                            {[
                                ...
                                // Add sticky class for symbol and name headers
                                let stickyClass = '';
                                if (col.key === 'symbol') stickyClass = styles.stickySymbolHeader;
                                else if (col.key === 'name') stickyClass = styles.stickyNameHeader;

                                return (
                                    <th
                                        key={col.key}
                                        className={`${styles.sortable} ${getSortClass(col.key)} ${stickyClass}`}
                        ...
                        <tr className={styles.filterRow}>
                            {visibleColumns.symbol && <td className={styles.stickySymbolFilter}><input type="text" placeholder="Filter..." value={filters.symbol} onChange={(e) => handleFilterChange('symbol', e.target.value)} className={styles.filterInput} /></td>}
                            {visibleColumns.name && <td className={styles.stickyNameFilter}><input type="text" placeholder="Filter..." value={filters.name} onChange={(e) => handleFilterChange('name', e.target.value)} className={styles.filterInput} /></td>}
                    ...
                    <tbody>
                        {filteredAndSortedStocks.map((stock: Stock) => {
                            ...
                            return (
                                <tr key={stock.symbol}>
                                    {visibleColumns.symbol && <td className={styles.stickySymbol}><Link href={`/stocks/${cleanSym}/financials?period=annual&country=Saudi Arabia`} className={styles.stockLink}>{cleanSym}</Link></td>}
                                    {visibleColumns.name && <td className={styles.stickyName}><Link href={`/stocks/${cleanSym}/financials?period=annual&country=Saudi Arabia`} className={styles.stockLink}>{cleanName(stock.name)}</Link></td>}
