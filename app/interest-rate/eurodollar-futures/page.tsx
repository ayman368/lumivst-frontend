...
export default function SofrFuturesPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/economic-indicators/eurodollar-futures/latest`)
      ...
      .catch(err => {
        console.error("Error fetching Eurodollar futures:", err);
        setLoading(false);
      });
  }, []);
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-slate-50">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2 text-lg">
          Eurodollar Futures — Source: Investing.com
        </p>
        {latestDate && (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="h-[400px] flex items-center justify-center text-gray-500 flex-col gap-4">
            <p className="text-xl">No Eurodollar futures data available yet.</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-right px-4 py-3 font-semibold text-gray-700">Last</th>
                  <th className="text-right px-4 py-3 font-semibold text-gray-700">Chg.</th>
                ...
                {data.map((row, i) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-2.5 text-right font-semibold ${
                      row.change === null ? 'text-gray-500' :
                      row.change > 0 ? 'text-green-600' :
                      row.change < 0 ? 'text-red-600' : 'text-gray-700'
                    }`}>
                      {fmtChg(row.change)}
                    </td>
