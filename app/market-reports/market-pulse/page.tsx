...
import { useTableSort, SortConfig } from "../_components/useTableSort";
...
interface SortEntry { col: string; dir: "asc" | "desc"; }
...
// ── Formatters ─────────────────────────────────────────────────────────────────
const n = (v: unknown, d = 2) => v == null ? "—" : Number(v).toLocaleString('en-US', { minimumFractionDigits: d, maximumFractionDigits: d });
...
const vol = (v: unknown) => v == null ? "—" : Number(v).toLocaleString('en-US', { maximumFractionDigits: 0 });
const chgColor = (v: unknown) => v == null ? "" : Number(v) >= 0 ? "positive" : "negative";

// Removed pulseBadge as we will handle Market Pulse cell directly

const outlookBadge = (v: unknown): { label: string; cls: string } | null => {
  if (!v) return null;
  const s = String(v);
  const map: Record<string, string> = {
    "FTD": "badge-ftd", "RD": "badge-rd", "PRD": "badge-prd",
    "DD": "badge-dd", "SD": "badge-sd"
  };
...
function MetricCard({ label, value, trend }: { label: string; value: string; trend?: "positive" | "negative" | "neutral" }) {
  const trendColor = trend === "positive" ? "#00875A" : trend === "negative" ? "#C0392B" : "#2C3E50";
  return (
    <div style={{ background: "#fff", border: "1px solid #E8ECF0", borderRadius: 6, padding: "10px 14px", minWidth: 120, flex: "0 0 auto", borderTop: `3px solid ${trendColor}` }}>
      <div style={{ fontSize: 10, color: "#6B7C93", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600, marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 15, fontWeight: 700, color: trendColor, fontFamily: "'IBM Plex Mono', monospace" }}>{value}</div>
    </div>
  );
}

// ── Sort Indicator ────────────────────────────────────────────────────────────
function SortIcon({ col, sorts }: { col: string; sorts: SortEntry[] }) {
  const idx = sorts.findIndex(s => s.col === col);
  if (idx === -1) return <span style={{ color: "#BDC3CC", marginLeft: 3, fontSize: 10 }}>⇅</span>;
  return (
    <span style={{ marginLeft: 3, display: "inline-flex", alignItems: "center", gap: 1 }}>
      <span style={{ color: "#F2A900", fontSize: 9, fontWeight: 700 }}>{idx + 1}</span>
      <span style={{ color: "#F2A900", fontSize: 11 }}>{sorts[idx].dir === "asc" ? "↑" : "↓"}</span>
    </span>
  );
}

// PulseIndicator removed
...
function Badge({ data }: { data: { label: string; cls: string } }) {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 7px", borderRadius: 3, fontSize: 11, whiteSpace: "nowrap", ...style }}>
      {data.label}
    </span>
  );
}
...
function SortIndicator({ sortConfigs, colKey }: { sortConfigs: SortConfig[]; colKey: string }) {
  ...
  if (idx === -1) return <span style={{ fontSize: 9, color: '#aaa', opacity: 0.5, lineHeight: '8px', marginLeft: 2 }}>▲<br />▼</span>;
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', marginLeft: 2, gap: 2 }}>
      <span style={{ fontSize: 10, fontWeight: 'bold' }}>{sortConfigs[idx].direction === 'asc' ? '▲' : '▼'}</span>
      {sortConfigs.length > 1 && (
        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 14, height: 14, background: '#2563eb', color: '#fff', fontSize: 9, fontWeight: 'bold', borderRadius: '50%' }}>
          {idx + 1}
        </span>
      )}
    </span>
...
export default function MarketPulsePage() {
  ...
  const { sortConfigs, handleSort, clearSort, sortedData: applySort } = useTableSort<MarketRecord>();
  ...
  const filteredRecords = useMemo(() => {
    return allRecords.filter(r => {
      ...
      if (yearFilter && yearFilter !== "" && String(r.year) !== yearFilter) return false;
      return true;
    });
  }, [allRecords, statusFilter, outlookFilter, yearFilter]);

  const sortedRecords = useMemo(() => applySort(filteredRecords), [filteredRecords, applySort]);
  const records = useMemo(() => sortedRecords.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE), [sortedRecords, page]);
  ...
  useEffect(() => {
    ...
    if (yearFilter && yearFilter !== "") params.set("year", yearFilter);
    fetch(`${API}/api/market-pulse/averages?${params}`, {
      headers: { "X-API-Key": process.env.NEXT_PUBLIC_API_KEY ?? "" },
    })
      .then(r => r.json()).then(setAvgs).catch(() => { });
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/market-pulse/stats?${params}`, {
      headers: { "X-API-Key": process.env.NEXT_PUBLIC_API_KEY ?? "" },
    })
      .then(r => r.json()).then(d => setTotal(d.total_records ?? 0)).catch(() => { });
  }, [statusFilter, outlookFilter, yearFilter]);
  ...
  const groupSpans: { group: string; span: number; color: string }[] = [];
  let lastGroup = ""; let span = 0;
  for (const col of COLS) {
    if (col.group !== lastGroup) {
      if (span > 0) groupSpans.push({ group: lastGroup, span, color: GROUP_COLORS[lastGroup] ?? "#2C3E50" });
      lastGroup = col.group; span = 1;
    } else { span++; }
  }
  if (span > 0) groupSpans.push({ group: lastGroup, span, color: GROUP_COLORS[lastGroup] ?? "#2C3E50" });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F0F2F5", fontFamily: "'IBM Plex Sans', 'Segoe UI', sans-serif" }}>
      ...
      {/* ── Top progress bar — no layout shift, purely cosmetic ── */}
      ...
      {/* ── Top navbar ── */}
      <div style={{ background: "#0D2B4B", borderBottom: "3px solid #F2A900", padding: "0 24px", display: "flex", alignItems: "center", height: 52, gap: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#F2A900" }} />
          <span style={{ color: "#fff", fontWeight: 700, fontSize: 14, letterSpacing: "0.04em" }}>MARKET PULSE</span>
        </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ color: "#A8C0D6", fontSize: 11 }}>{filteredRecords.length.toLocaleString()} records</span>
          {sortConfigs.length > 0 && (
            <button
              onClick={() => clearSort()}
              style={{ background: "#F2A900", color: "#0D2B4B", border: "none", borderRadius: 4, padding: "4px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
            >
      ...
      <div style={{ maxWidth: 1600, margin: "0 auto", padding: "16px 24px", display: "flex", flexDirection: "column", gap: 14 }}>

        {/* ── Filter bar ── */}
        <div style={{ background: "#fff", border: "1px solid #E0E6ED", borderRadius: 8, padding: "12px 16px", display: "flex", flexWrap: "wrap", gap: 16, alignItems: "flex-end" }}>
          <div>
            <label style={labelStyle}>Market Pulse Status</label>
            <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(0); }} style={{ ...inputStyle, paddingRight: 28, minWidth: 180 }}>
          ...
          <div>
            ...
            <select value={outlookFilter} onChange={e => { setOutlookFilter(e.target.value); setPage(0); }} style={{ ...inputStyle, minWidth: 100 }}>
          ...
          <div>
            ...
            <input type="number" placeholder="e.g. 2024" value={yearFilter} onChange={e => { setYearFilter(e.target.value); setPage(0); }} style={{ ...inputStyle, width: 90 }} />
          </div>
          {hasFilters && (
            <button onClick={clearFilters} style={{ background: "#fff", border: "1px solid #D1D9E0", color: "#4A5568", borderRadius: 5, padding: "6px 14px", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>
              ✕ Clear Filters
            </button>
          )}
          <div style={{ marginLeft: "auto", fontSize: 10, color: "#AAB4BE", alignSelf: "center" }}>
        ...
        {avgs && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "4px 0" }}>
            ...
            ] as { label: string; value: string; trend: "positive" | "negative" | "neutral" }[]).map(c => (
        ...
        {/* ── Error ── */}
        {error && (
          <div style={{ background: "#FDEDEC", border: "1px solid #E74C3C", borderRadius: 8, padding: "12px 16px", color: "#7B241C", fontSize: 13 }}>
            ⚠ Failed to load data: {error}
          </div>
        )}

        {/* ── Table wrapper ── */}
        <div style={{ background: "#fff", border: "1px solid #E0E6ED", borderRadius: 8, overflow: "hidden", position: "relative" }}>

          {/* Table stays visible, no dimming overlay since we do client-side sorting */}

          {initialLoad ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 200, color: "#6B7C93", gap: 10 }}>
              <div style={{ width: 20, height: 20, border: "2px solid #0D2B4B", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
              Loading market data…
            </div>
          ) : (
            <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 400px)", minHeight: 400 }}>
              <table style={{ borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed", minWidth: "max-content" }}>
                <thead>
                  <tr>
                    <th style={{ position: "sticky", top: 0, left: 0, zIndex: 30, background: "#0D2B4B", color: "#A8C0D6", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "4px 10px", borderRight: "2px solid #F2A900", borderBottom: "1px solid #1A4070", textAlign: "left", minWidth: 96, whiteSpace: "nowrap" }}>
                      Date
                    </th>
                    {groupSpans.map((g, i) => (
                      <th key={i} colSpan={g.span} style={{ position: "sticky", top: 0, zIndex: 20, background: g.color, color: "#ffffffCC", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "4px 8px", borderRight: "1px solid rgba(255,255,255,0.15)", borderBottom: "1px solid rgba(255,255,255,0.2)", textAlign: "center", whiteSpace: "nowrap" }}>
                        {g.group}
                      </th>
                    ))}
                  </tr>
                  <tr style={{ background: "#F7F9FC" }}>
                    <th onClick={() => handleSort("date")} style={{ position: "sticky", top: 28, left: 0, zIndex: 30, background: "#EFF2F6", cursor: "pointer", padding: "7px 10px", borderRight: "2px solid #D0D7E0", borderBottom: "2px solid #C8D0DA", fontSize: 11, fontWeight: 700, color: "#2C3E50", textAlign: "left", whiteSpace: "nowrap", minWidth: 96 }}>
                      <div className="flex items-center justify-start gap-1">Date <SortIndicator sortConfigs={sortConfigs} colKey="date" /></div>
                    </th>
                    {COLS.map(col => (
                      <th key={col.key} onClick={col.sortable ? () => handleSort(col.key) : undefined}
                        style={{ position: "sticky", top: 28, zIndex: 20, background: "#F7F9FC", cursor: col.sortable ? "pointer" : "default", padding: "7px 8px", borderRight: "1px solid #E8ECF0", borderBottom: "2px solid #C8D0DA", fontSize: 11, fontWeight: 700, color: "#4A5568", textAlign: col.align === "right" ? "right" : "left", whiteSpace: "nowrap", minWidth: 70, userSelect: "none" }}>
                        <div className={`flex items-center ${col.align === "right" ? "justify-end" : "justify-start"} gap-1`}>
                          {col.label}{col.sortable && <SortIndicator sortConfigs={sortConfigs} colKey={col.key} />}
                        </div>
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {records.map((row, ri) => {
                    ...
                    return (
                      ...
                      >
                        <td style={{ position: "sticky", left: 0, zIndex: 10, background: "inherit", borderRight: "2px solid #D0D7E0", borderBottom: "1px solid #EDF0F4", padding: "6px 10px", whiteSpace: "nowrap", minWidth: 96 }}>
                          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, color: "#1B2A3B", fontWeight: 500 }}>{row.date}</div>
                        </td>
                        {COLS.map(col => {
                          const raw = row[col.key as keyof MarketRecord];
                          
                          if (col.key === "market_pulse") {
                            const pBg = raw === "Confirmed uptrend" ? "#D4EFDF" :
                                        raw === "Uptrend under pressure" ? "#FDEBD0" :
                                        raw === "Market in correction" ? "#FADBD8" : undefined;
                            return (
                              <td key={col.key} style={{ background: pBg, color: pBg ? "#000" : undefined, borderRight: "1px solid #EDF0F4", borderBottom: "1px solid #EDF0F4", padding: "6px 8px", textAlign: "left", whiteSpace: "nowrap" }}>
                                {raw ? String(raw) : "—"}
                              </td>
                            );
                          }
                          ...
                          if (col.badge) {
                            ...
                            return (
                              <td key={col.key} style={{ borderRight: "1px solid #EDF0F4", borderBottom: "1px solid #EDF0F4", padding: "6px 8px", textAlign: "left", whiteSpace: "nowrap" }}>
                                {data ? <Badge data={data} /> : <span style={{ color: "#CDD3DA", fontSize: 11 }}>—</span>}
                              </td>
                            );
                          }
                          const text = col.fmt ? col.fmt(raw) : (raw == null ? "—" : String(raw));
                          const colorKey = col.color ? col.color(raw) : "";
                          const textColor = colorKey === "positive" ? "#00875A" : colorKey === "negative" ? "#C0392B" : "#2C3E50";
                          const isNeutral = text === "—";
                          return (
                            <td key={col.key} style={{ borderRight: "1px solid #EDF0F4", borderBottom: "1px solid #EDF0F4", padding: "6px 8px", textAlign: col.align ?? "right", whiteSpace: "nowrap", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, color: isNeutral ? "#CDD3DA" : textColor, fontWeight: isNeutral ? 400 : 500 }}>
                              {text}
                            </td>
                          );
                        })}
        ...
        {!initialLoad && !error && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 0" }}>
            <button
              ...
              style={{ background: "#fff", border: "1px solid #D1D9E0", color: page === 0 ? "#BDC3CC" : "#0D2B4B", borderRadius: 5, padding: "7px 18px", fontSize: 12, fontWeight: 600, cursor: page === 0 ? "not-allowed" : "pointer" }}
            >
              ← Previous
            </button>
            <span style={{ fontSize: 12, color: "#6B7C93" }}>
              Page <strong>{page + 1}</strong> · Showing <strong>{page * PAGE_SIZE + 1}–{page * PAGE_SIZE + records.length}</strong> of <strong>{filteredRecords.length.toLocaleString()}</strong> records
            </span>
            <button
              disabled={records.length < PAGE_SIZE}
              onClick={() => setPage(p => p + 1)}
              style={{ background: "#0D2B4B", border: "1px solid #0D2B4B", color: records.length < PAGE_SIZE ? "#BDC3CC" : "#fff", borderRadius: 5, padding: "7px 18px", fontSize: 12, fontWeight: 600, cursor: records.length < PAGE_SIZE ? "not-allowed" : "pointer" }}
            >
              Next →
            </button>
          </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        tbody tr:hover td[style*="sticky"] { background: #EEF4FB !important; }
      `}</style>
    </div>
  );
}